  ...
  try {
    for (let i = 0; i < retries; i++) {
      try {
        ...
        const stored = await chrome.storage.session.get(['sessionSeed', 'sessionStartTime', 'profile', 'timestamp']);
...
async function getSessionInfo() {
  try {
    ...
    // Si no hay perfil pero tenemos seed, esperar un poco
    if (!sessionData.profile && !stored?.profile && sessionData.seed) {
      console.log('[Chameleon] Waiting for profile generation...');
      
      // Crear una promesa que se resuelve cuando llegue el perfil
      const profilePromise = new Promise((resolve) => {
        profileWaiters.push({ resolve });
        
        // Timeout después de 10 segundos (más tiempo para generación inicial)
        setTimeout(() => {
          resolve(null);
        }, 10000);
      });
