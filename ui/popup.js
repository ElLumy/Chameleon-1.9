...
(() => {
    ...
    const PROFILE_WAIT_TIME = 15000; // 15 seconds max wait for profile
    ...
    script.src = 'consistency-checker.js';
        ...
        try {
            ...
            if (currentProfile) {
                ...
                if (profile) {
                    ...
                    throw new Error('Profile generation timeout');
                }
            }
            ...
            if (error.message.includes('Profile generation timeout')) {
                showError('Profile generation is taking longer than expected. Please wait...');
                // Continue retrying
                if (initRetries < MAX_INIT_RETRIES) {
                    initRetries++;
                    setTimeout(() => init(), 2000);
                }
            } else if (error.message.includes('No session seed')) {
    ...
    async function waitForProfile() {
        ...
        while (Date.now() - startTime < PROFILE_WAIT_TIME) {
            try {
                ...
                console.warn('[Chameleon Popup] Error checking for profile:', error);
            }
    ...
    function showNoProfile() {
        if (elements.profileName) elements.profileName.textContent = 'No Profile';
        if (elements.profileLocation) elements.profileLocation.textContent = 'Not initialized';
        if (elements.profileDevice) elements.profileDevice.textContent = 'Unknown';
        if (elements.sessionSeed) elements.sessionSeed.textContent = 'No session';
    ...
    async function checkCoherence() {
        ...
        try {
            // Use ConsistencyChecker if available
            if (typeof ConsistencyChecker !== 'undefined') {
                const checker = new ConsistencyChecker();
                const results = await checker.checkProfile(currentProfile);
                
                // Map results to UI format
                const uiChecks = {};
                Object.entries(results.checks).forEach(([id, check]) => {
                    uiChecks[id] = {
                        status: check.status,
                        score: check.score,
                ...
                updateCoherenceUI(uiChecks);
                coherenceScore = results.percentage;
                updateScoreUI();
            } else {
    ...
    // Simple coherence check as fallback
